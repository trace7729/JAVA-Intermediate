...
private enum toPerform {CREATE, RETRIEVE, UPDATE, DELETE,CONTENT};
...
public void setView(InventoryView2 view) {
	this.view = view;
	}
...
public String[] getMediaColumnArray(){
	return mediaType.getColumnArray();
}
...
public void createItem(String createString){
	setNextString(inventoryObjects[getInventoryObjectsIndex()].createItem(createString));
	}
	
public void retrieveItem(String retrieveString){
	inventoryObjects[getInventoryObjectsIndex()].retrieveItem(retrieveString);
	setNextString(inventoryObjects[getInventoryObjectsIndex()].retrieveItem(retrieveString));
	}

public void updateItem(int num, String key, String updateString){
	setNextString(inventoryObjects[getInventoryObjectsIndex()].updateItem(num, key, updateString));  
	}
	
public void deleteItem(String deleteString){
	setNextString(inventoryObjects[getInventoryObjectsIndex()].deleteItem(deleteString));
	}
public void listAllProperties(){
	String content=inventoryObjects[getInventoryObjectsIndex()].listItem();
	setNextString(content);
}
//method setSelectedOperation(int operation) sets operation in model
// and updates view with an integer
public void setSelectedOperation(int operation) {
	if (operation == 1)
		...
		currentOperation=toPerform.CONTENT;
...
public void performOperation(){
	...
	else if (currentOperation == toPerform.CONTENT)
